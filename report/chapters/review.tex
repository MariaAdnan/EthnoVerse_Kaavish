...
\subsection{Gaussian Splatting (3DGS)}
3D Gaussian Splatting (3DGS) represents a recent advancement in real-time radiance field rendering, introduced as an alternative to computationally intensive NeRF models. Instead of encoding the scene in a neural network, 3DGS represents it as a collection of anisotropic Gaussian primitives in 3D space. Each Gaussian encodes position, color, opacity, and covariance, allowing efficient rasterization directly on the GPU.
Compared to NeRF, 3DGS offers:

\begin{itemize}
    \item \textbf{Real-time rendering:} Enables interactive frame rates suitable for web-based visualization
    \item \textbf{Faster training times:} Scene optimization completes in minutes instead of hours
    \item \textbf{Explicit representation:} Easier integration with graphics pipelines like Three.js
    \item \textbf{Lower computational overhead:} More feasible for mid-range GPU setups
\end{itemize}
...
\begin{itemize}
    ...
    \item integrating state-of-the-art Gaussian Splatting methods within a 
    cultural archival workflow,
    \item designing a scalable platform capable of supporting multiple 
    communities,
