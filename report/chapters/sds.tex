...
\begin{itemize}
    ...
    \item \textbf{methodUsed}: Reconstruction method (\texttt{3DGS} as primary, \texttt{NeRF} as baseline).
...
\begin{enumerate}
  ...
  \item \textbf{Asynchronous Workers}:
    \begin{itemize}
      ...
      \item \textbf{3D Build (optional)}: enqueue scene build job (3D Gaussian Splatting) if user requests a 3D reconstruction from a set of images.    \end{itemize}
\item \textbf{3D Reconstruction}: GPU worker pulls images from storage, computes camera poses (COLMAP or similar), and performs 3D Gaussian Splatting (3DGS) optimization. The system generates a set of Gaussian primitives representing the scene, along with preview renders. Optimized scene data, metadata, and performance statistics are stored in the database.
...
\subsection{3D Gaussian Splatting (3DGS) Reconstruction Pipeline}

\subsubsection{3D Gaussian Splatting Algorithm}

3D Gaussian Splatting (3DGS) is a recent real-time neural rendering technique that represents a scene as a collection of anisotropic Gaussian primitives instead of encoding it in a neural network. This explicit representation enables significantly faster rendering and training compared to Neural Radiance Fields (NeRF), making it suitable for interactive applications.

\subsubsection*{Scene Representation}

In 3DGS, a scene is modeled as a set of Gaussian primitives:
...
where:
\begin{itemize}
    \item $\mu_i$: 3D position of the Gaussian
    \item $\Sigma_i$: covariance matrix defining shape and orientation
    \item $c_i$: RGB color
    \item $\alpha_i$: opacity
\end{itemize}

Each Gaussian contributes to the final rendered image through alpha blending.

\subsubsection*{Input Requirements}

The system requires:
\begin{itemize}
    \item Multi-view RGB images (20–200 images with overlap)
    \item Camera intrinsics and extrinsics (computed via COLMAP)
\end{itemize}

\subsubsection*{Initialization}

A sparse point cloud generated from Structure-from-Motion (SfM) is used to initialize Gaussian positions. Each point is converted into an initial Gaussian primitive.

\subsubsection*{Optimization}

Gaussian parameters are optimized using gradient-based methods to minimize reconstruction error:

\[
\mathcal{L} = \sum || I_{\text{rendered}} - I_{\text{groundtruth}} ||^2
\]
...
Unlike NeRF, which relies on volumetric ray marching, 3DGS uses rasterization-based rendering. Each Gaussian is projected onto the image plane and contributes to pixel color using alpha compositing.

This enables:
\begin{itemize}
    \item Real-time rendering (30–60 FPS)
    \item Efficient GPU utilization
    \item Low latency interaction
\end{itemize}


\subsubsection{3DGS Reconstruction Pipeline}
\textbf{Purpose:} reconstruct immersive 3D scenes from multi-angle field photos for real-time virtual exploration.

\textbf{Inputs:}
\begin{itemize}
  \item Image set for scene (20–200 overlapping images),
  \item Camera intrinsics and extrinsics (estimated via COLMAP),
  \item Optional metadata for scene organization.
\end{itemize}

\textbf{Outputs:}
\begin{itemize}
  \item Optimized Gaussian-based 3D representation,
  \item Real-time renderable scene for web viewer,
  \item Preview images and metadata (camera pose summary, performance stats),
...
\begin{enumerate}
  ...
  \item Camera pose estimation (Structure-from-Motion)
  \item Gaussian initialization from sparse point cloud
  \item Optimization of Gaussian parameters
  \item Real-time rendering integration (Three.js viewer)
\end{enumerate}