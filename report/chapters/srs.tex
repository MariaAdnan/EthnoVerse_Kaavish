...
This section specifies the functional requirements of the system. The requirements are divided into two primary categories: those pertaining to the \textbf{web platform} and those related to the \textbf{3D virtual tour module} developed using Guassian Splatting. Each subsection includes a tabular summary and a module-wise functional outline for clarity.
...
\begin{outline}
  \1 \textbf{Module 1: Data Preparation}
    ...
    \2 Preprocess images (resize, align, normalize lighting) before 3DGS input.

  \1 \textbf{Module 2: Scene Reconstruction}
    \2 Implement 3D Gaussian Splatting (primary) with NeRF explored as a baseline method.
...
\textbf{ID} & \textbf{Function Name} & \textbf{Description} & \textbf{Users} \\
\hline
FR-3D01 & Dataset Preparation & Collect and preprocess multiple-angle photographs of community spaces for Guassian Splats input. & Admin \\
FR-3D02 & Scene Reconstruction & Generate 3D radiance field representations using Gaussian Splatting techniques. & System \\
...
FR-3D06 & Fallback Handling & If reconstruction fails, fallback to photo-based 360° panoramic tour is generated. & System \\
\hline
\end{tabular}
...
This section includes our mockup screens and briefly explains them. The platform provides two main user-facing interfaces:
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{3.png} % Search Communities - 3.png
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{4.png} % Community Detail - 4.png
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{5.png} % 3D Viewer - 5.png
...
\vspace{0.2cm}
\par
The viewer provides an immersive, navigable virtual tour experience of the reconstructed community environment. The screen hosts the interactive 3D rendering, generated using Gaussian Splatting, which is optimized for web deployment. A supporting section, \texttt{About This Tour}, provides context on the ethnographic and technical basis of the reconstruction. 

\vspace{0.2cm}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{6.png} % Media Detail (Image) - 6.png
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{7.png} % Media Detail (Audio) - 7.png
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{8.png} % About Page - 8.png
...
The system is built using a \textbf{RESTful API} architecture (Node.js/Express backend) to manage data flow between the React frontend, the Supabase database, and the Python-based 3D Reconstruction Engine.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \texttt{POST /api/3d/reconstruct}: (Admin/System) Triggers the Gaussian Splatting engine for a new scene reconstruction.
...
\item \textbf{Server-Side Infrastructure:} The backend and database are hosted on a cloud environment, supporting modular scaling.
...
\noindent
The EthnoVerse architecture follows a modular, service-oriented design built with a dedicated 3D reconstruction module. Figure~\ref{fig:systemblock} illustrates the high-level components and data flow between them.
...
\subsubsection*{3. Database (Supabase)}
Supabase stores all structured metadata, user profiles, and references to media files. The schema supports flexible, community-specific expansion and version-controlled updates. It also logs administrative actions and maintains backup copies for recovery and integrity checks.

\subsubsection*{4. Media Storage Service}
All uploaded media—images, audio, video, and 3D assets—are stored in a cloud-based repository. Metadata in Supabase links each file to its respective community and media type. The storage layer supports redundancy and retrieval optimization for faster access and rendering.

\subsubsection*{5. 3D Reconstruction Engine (Gaussian Splatting)}
...
\begin{itemize}
    ...
    \item The backend retrieves or updates records in Supabase and media storage.
