  ...
  const handleDownload = async () => {
    ...
    try {
      await downloadRemoteFile(
        item.picture_cloudinary_url,
        `${item.title.replace(/\s+/g, "_")}.jpg`,
      );
      toast.success("Image download started.");
    } catch (downloadError) {
      toast.error(errorMessage(downloadError, "Image download failed."));
    }
  };

  // ─── Render ───────────────────────────────────────────────────────────────
  return (
    <div className="fixed inset-0 bg-ink/95 z-50 flex items-center justify-center p-4 sm:p-8">
      ...
      <motion.button
        ...
        aria-label="Close"
        className="absolute top-4 right-4 sm:top-8 sm:right-8 w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center border border-paper/30 hover:border-accent hover:bg-accent/10 transition-colors text-paper z-10 shrink-0"
      >
      ...
      <div className="absolute top-4 left-4 sm:top-8 sm:left-8 z-10 pointer-events-none">
        <p className="text-xs text-paper/40" style={{ fontFamily: "'Space Mono', monospace" }}>
      ...
      {siblingIds.length > 1 && (
        <motion.button
          ...
          className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center border border-paper/30 hover:border-accent hover:bg-accent/10 transition-colors text-paper z-10 disabled:pointer-events-none"
        >
      ...
      {siblingIds.length > 1 && (
        <motion.button
          ...
          className="absolute right-14 sm:right-20 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center border border-paper/30 hover:border-accent hover:bg-accent/10 transition-colors text-paper z-10 disabled:pointer-events-none"
        >
      ...
      <AnimatePresence mode="wait">
        ...
        {loading && (
          <motion.div
            ...
            className="flex flex-col items-center gap-4 text-paper/60"
          >
        ...
        {!loading && error && (
          <motion.div
            ...
            className="flex flex-col items-center gap-4 text-paper/70 max-w-sm text-center px-6"
          >
        ...
        {!loading && !error && item && (
          ...
          >
            ...
            >
              <div className="backdrop-blur-md bg-ink/80 border border-paper/20 p-4 sm:p-6">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    <p
                      className="text-xs text-paper/60 mb-1 sm:mb-2 truncate"
                      style={{ fontFamily: "'Space Mono', monospace" }}
                    >
                    ...
                    <h2
                      className="text-lg sm:text-2xl text-paper leading-tight"
                      style={{ fontFamily: "'Playfair Display', serif" }}
                    >
                      {item.title}
                    </h2>
                    {item.description ? (
                      <p className="text-xs sm:text-sm text-paper/70 mt-1 sm:mt-2 line-clamp-2">
                        {item.description}
                      </p>
                    ) : item.communities?.name ? (
                      <p className="text-xs sm:text-sm text-paper/70 mt-1 sm:mt-2">
                  ...
                  <button
                    ...
                    className="shrink-0 w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center border border-paper/30 hover:border-accent hover:bg-accent/10 transition-colors text-paper"
                  >
                ...
                <div
                  className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-paper/10 grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 text-xs text-paper/60"
                  style={{ fontFamily: "'Space Mono', monospace" }}
                >
                  <div>
                    <p className="mb-1">DATE</p>
                    <p className="text-paper/90">{formatDate(item.created_at)}</p>
                  </div>
                  <div>
                    <p className="mb-1">LOCATION</p>
                    <p className="text-paper/90">{item.communities?.location ?? "—"}</p>
                  </div>
                  {item.tags && item.tags.length > 0 && (
                    <div className="col-span-2 sm:col-span-1">
                      <p className="mb-1">TAGS</p>
                      <p className="text-paper/90 truncate">{item.tags.join(", ")}</p>
                    </div>
                  )}
                </div>
              </div>
